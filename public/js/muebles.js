			...
			success:function(respuesta){
				...
				$.each(respuesta, function(i, item) {
					cadena = cadena + "<tr>"+
											...
											"<td>" + item.fecha + "</td>" +
											'<td>'+
												'<span class="btn btn-dark btn-sm" data-toggle="modal" data-target="#modalActualizarMueble" '+
												' onclick="obtenerIdMueble(' + item.id_mueble + ')">'+
													'<span class="fas fa-user-times"></span>'+
												'<span>'+
											'</td>' +
											'<td>'+
												'<span class="btn btn-warning btn-sm" onclick="eliminarMueble(' + item.id_mueble + ')">'+
	...
	$.ajax({
			...
			success:function(respuesta){
				...
				if (respuesta) {
					...
					swal("Error de Actualizar!¡", "error");
				}
			}
		});
